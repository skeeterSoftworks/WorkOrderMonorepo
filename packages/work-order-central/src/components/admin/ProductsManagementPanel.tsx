...
export function ProductsManagementPanel() {
    ...
    const openTechnologyModal = (product: LocalProduct) => {
        ...
        const td = latest.technologyData;
        setTechnologyDraft({
            id: td?.id,
    ...
    return (
        <Box sx={{ mt: 3 }}>
            ...
            <Dialog open={technologyModalOpen} onClose={closeTechnologyModal} maxWidth="md" fullWidth>
                <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    {t('technologyEditorTitle')}
                    <IconButton size="small" onClick={closeTechnologyModal} aria-label={t('close')}>
                        <CloseIcon />
                    </IconButton>
                </DialogTitle>
                <DialogContent dividers>
