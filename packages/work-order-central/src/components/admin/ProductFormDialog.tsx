...
export function ProductFormDialog({ open, product, machines, customers, onClose, onSaved }: Props) {
    ...
    const handleSubmit = () => {
        ...
        const onSuccess = () => {
            onSaved();
            onClose();
            toastActionSuccess(product?.id ? t('toastProductUpdated') : t('toastProductAdded'));
        };
        if (product?.id) {
        ...
        <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth scroll="paper">
            ...
            <DialogContent dividers>
                <Box component="form" autoComplete="off" sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
                    ...
                    >
                        {customers.map((c) => (
                            <MenuItem key={c.id} value={c.id} sx={multiSelectMenuItemSx}>
                                {c.companyName}
                            </MenuItem>
                        ))}
